"""
Token-level dataset that aligns JSONL token labels with cached hidden-state features.

Default dataset behavior:
- Scans feature files from `feature_splits=('train', 'validation', 'test')` under `root`.
- Reads token labels from filtered/augmented JSONL and matches candidates by index,
  with optional fingerprint fallback (`match_mode='auto'`).
- Drops whitespace/comment tokens by default (`drop_ws_comment=True`).
- Returns `(features, label, metadata)` where metadata includes line number and line label.
- Binary label convention is hallucination-positive: correct token -> `0.0`, incorrect -> `1.0`.

Additional options:
- `layers`: fixed layer subset; if omitted, inferred from feature payload.
- `combiner`: `concat` (default) or `stack` for per-layer feature layout.
- Mapping controls: `match_mode`, `strict_mapping`, `warn_on_collision`,
  `expected_jsonl_split`, and `split_priority`.
"""
...
class TokenLevelHiddenStateDataset(Dataset):
	def __init__(
		self,
		root: Path,
		jsonl_path: Path,
		...
		self._items: List[TokenSampleIndex] = []
		...
		self.stats: Dict[str, int] = {
			...
			"token_labels_total": 0,
			"token_labels_kept": 0,
			"tokens_dropped_ws_comment": 0,
			"tokens_dropped_no_label": 0,
		...
		self._build_token_items()

		if not self._items:
			raise RuntimeError("No usable token-level samples found.")
		...
		LOGGER.info(
			"Loaded token dataset from %s with %s token samples (rows=%s, matched_candidates=%s, unmatched_candidates=%s)",
			self.jsonl_path,
			len(self._items),
	...
	def __getitem__(self, index: int) -> Tuple[torch.Tensor, torch.Tensor, Dict[str, torch.Tensor]]:
		...
		for layer in self.layers:
			...
			vector = self._lookup_vector(layer_dict, item.token_position)
			if vector is None:
				raise KeyError(
					f"Token position {item.token_position} missing for layer {layer} in {item.path}"
				)
		...
		metadata = {
			...
			"line_label": torch.tensor(item.line_label, dtype=torch.int8),
		}
	...
	def _build_token_items(self) -> None:
		for record in self._iter_jsonl(self.jsonl_path):
			...
			for json_candidate_idx, token_labels in enumerate(token_labels_raw):
				...
				for token_offset, token_info in enumerate(token_labels):
					...
					if self.hidden_size is None:
						self.hidden_size = self._infer_hidden_size(hidden_states, candidate_layers, token_position)
						if self.hidden_size is not None:
							self.feature_dim = self.hidden_size * len(candidate_layers) if self.combiner == "concat" else self.hidden_size
					label_float = 0.0 if bool(label_value) else 1.0
					self._items.append(
						TokenSampleIndex(
							path=entry.path,
					...
					self.stats["token_labels_kept"] += 1
					candidate_kept += 1
				...
				if candidate_kept > 0:
					...
					self._handle_unmatched(
						f"No usable token labels after filtering for sample_id={sample_id} candidate_idx={json_candidate_idx}"
					)
