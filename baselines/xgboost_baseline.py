"""
Train an XGBoost baseline on flattened hidden-state features.

Default training behavior:
- Loads `train` and `validation` splits from `--data-root`.
- Uses layer `[48]` and token position `[0]` unless overridden.
- Uses `concat` feature combiner and keeps empty code spans unless `--filter-empty`.
- Saves the best booster to `outputs/xgboost/model.json`.

Important options:
- Feature selection: `--layers`, `--token-positions`, `--combiner`, `--filter-empty`.
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--filter-empty", action="store_true", help="Whether to filter out candidates with empty code token spans")
    parser.add_argument(
        "--combiner",
