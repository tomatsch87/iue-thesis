"""
Train a line-level XGBoost baseline from cached hidden states.

Default training behavior:
- Requires `--train-jsonl` and `--valid-jsonl` aligned with feature files.
- Uses layers `[48]` and feature splits `train/validation/test` by default.
- Line feature mode defaults to `--line-representation last_token` and
	`--line-aggregation mean`.
- Converts each line sample to a flattened vector and trains binary logistic XGBoost.
- Automatically computes `scale_pos_weight` from class counts unless explicitly set.
- Saves model to `outputs/token_level/line_xgboost/model.json`.

Important options:
- Data/feature controls: `--layers`, `--feature-splits`, `--line-representation`,
	`--line-aggregation`, `--limit-train`, `--limit-valid`.
...
def _parse_args() -> argparse.Namespace:
	parser = argparse.ArgumentParser(
		description="Train an XGBoost line-level hallucination detector on cached hidden-state features",
	)
	parser.add_argument(
		"--data-root",
		type=Path,
	...
	parser.add_argument("--limit-train", type=int, help="Optional cap on the number of training line samples")
	parser.add_argument("--limit-valid", type=int, help="Optional cap on the number of validation line samples")
	parser.add_argument(
		"--scale-pos-weight",
		type=float,
	...
	parser.add_argument(
		"--model-path",
		type=Path,
		default=Path("outputs/token_level/line_xgboost/model.json"),
...
def _log_dataset_stats(prefix: str, dataset: LineLevelHiddenStateDataset) -> None:
...
def main() -> None:
	...
	LOGGER.info("Loading line datasets from feature root %s", args.data_root)
