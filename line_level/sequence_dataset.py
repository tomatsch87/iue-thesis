"""
Line-sequence dataset for models that consume per-line token sequences.

Default dataset behavior:
- Scans feature files from `feature_splits=('train', 'validation', 'test')`.
- Aligns JSONL rows/candidates to cached features with `match_mode='auto'`.
- Builds one sequence per line and returns `(line_sequence, label, metadata)`.
- Drops whitespace/comment-only lines by default (`drop_ws_comment=True`,
  `include_ws_comment_tokens=False`).
- Label convention is hallucination-positive: clean/correct line -> `0.0`,
  line with errors -> `1.0`.

Additional options:
- `layers` fixed or inferred from payload.
- `combiner`: `concat` (default) or `stack` for per-token layer composition.
...
class LineSequenceHiddenStateDataset(Dataset):
	def __init__(
		self,
		root: Path,
		jsonl_path: Path,
		...
		self.include_ws_comment_tokens = bool(include_ws_comment_tokens)
		...
		self._items: List[LineSequenceSampleIndex] = []
		...
		self.stats: Dict[str, int] = {
			...
			"line_sequences_total": 0,
			"line_sequences_kept": 0,
			"line_labels_total": 0,
			"line_labels_kept": 0,
			"lines_dropped_missing_label": 0,
			"lines_dropped_ws_comment_only": 0,
		...
		self._build_sequence_items()

		if not self._items:
			raise RuntimeError("No usable line-sequence samples found.")
		...
		LOGGER.info(
			"Loaded line-sequence dataset from %s with %s sequences (rows=%s, matched_candidates=%s, unmatched_candidates=%s)",
			self.jsonl_path,
			len(self._items),
	...
	def __getitem__(self, index: int) -> Tuple[torch.Tensor, torch.Tensor, Dict[str, torch.Tensor]]:
		...
		for token_position in item.token_positions:
			per_layer_vectors: List[torch.Tensor] = []
			for layer in self.layers:
				...
				if vector is None:
					raise KeyError(f"Token position {token_position} missing for layer {layer} in {item.path}")
				per_layer_vectors.append(vector.to(torch.float32))
			line_vectors.append(self._combine_vectors(per_layer_vectors))

		features = torch.stack(line_vectors, dim=0)
	...
	def _build_sequence_items(self) -> None:
		for record in self._iter_jsonl(self.jsonl_path):
			...
			for json_candidate_idx, token_labels in enumerate(token_labels_raw):
				...
				line_all_positions: Dict[int, List[int]] = {}
				line_non_ws_positions: Dict[int, List[int]] = {}
				line_non_ws_token_correctness: Dict[int, List[bool]] = {}
				...
				for token_offset, token_info in enumerate(token_labels):
					...
					if token_position > code_end:
						...
						continue
					is_ws_comment = bool(token_info.get("ws_comment", False))
					line_all_positions.setdefault(line_number, []).append(token_position)
					if not is_ws_comment:
						line_non_ws_positions.setdefault(line_number, []).append(token_position)
						if "label" in token_info and token_info.get("label") is not None:
							line_non_ws_token_correctness.setdefault(line_number, []).append(bool(token_info.get("label")))

				self.stats["line_sequences_total"] += 1
				candidate_kept = 0

				for line_number in sorted(line_all_positions.keys()):
					self.stats["line_labels_total"] += 1
					non_ws_positions = line_non_ws_positions.get(line_number, [])
					if self.drop_ws_comment and not non_ws_positions:
						self.stats["lines_dropped_ws_comment_only"] += 1
						continue

					if self.include_ws_comment_tokens and line_all_positions.get(line_number):
						selected_positions = list(line_all_positions[line_number])
					else:
						selected_positions = list(non_ws_positions)
					...
					for token_position in selected_positions:
						if self._has_required_vectors(hidden_states, candidate_layers, token_position):
							filtered_positions.append(token_position)
						else:
					...
					if line_label < 0:
						line_label = self._derive_line_label_from_tokens(
							line_non_ws_token_correctness.get(line_number, [])
						)
					if line_label < 0:
					...
					if self.hidden_size is None:
						self.hidden_size = self._infer_hidden_size(hidden_states, candidate_layers, filtered_positions[0])
					...
					self._items.append(
						LineSequenceSampleIndex(
							path=entry.path,
							candidate_key=candidate_key,
							line_number=int(line_number),
							label=float(line_label),
							token_positions=tuple(filtered_positions),
						)
					)
				...
				if candidate_kept > 0:
					self.stats["line_sequences_kept"] += 1
